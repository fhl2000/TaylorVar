### grad utils functions, inspired from the DeepXDE project.
        ...
        if i not in self.J:
            ...
            try: 
                self.J[i] = torch.autograd.grad(
                    y, self.xs, grad_outputs=torch.ones_like(y), create_graph=True, retain_graph=True
                )[0]
            except:
                self.J[i] = torch.zeros_like(self.xs)
...
def hessian_all(y,xs,grad_y=None):
  """
  no-lazy implentmention of hessian
  """
  dim=xs.shape[1]
  result=[]
  if grad_y is None:
    grad_y = jacobian(y, xs, i=0, j=None)
  for i in range(dim):
    for j in range(dim):
      result.append(hessian(y,xs,i=i,j=j,grad_y=grad_y))
  result=torch.cat(result,dim=1).reshape([-1,dim,dim])
  return result
